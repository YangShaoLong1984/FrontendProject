...
> - 垃圾收集器在运行的时候会给存储在内存中的所有变量`都加上标记`。然后，它会`去掉`环境中的`变量`以及被环境中的变量引用的`标记`。而在此之后`再被加上标记`的变量将被视为准备删除的变量，原因是环境中的变量已经无法访问到这些变量了。最后。垃圾收集器完成内存清除工作，销毁那些带标记的值，并回收他们所占用的内存空间。
>
> **2）引用计数**
>
