...
> ```
>
> 首先明确，队列的api：push shift    尾部进， 头部出
>
> 接下来用一个队列来实现：
>
...
>             stack.push(a);
>         } else {
>             stack.push(map.get(a)(stack.pop(), stack.pop()));
>         }
>     }
>     return stack.pop();
> };
...
> 单调队列的特性api：
>
> `push(n)`:入队。单调队列的push操作要保持队列的单调（本题队头为起点单调递增）,push进一个值n后，n要一直往前走（把比它小的全删除），知道遇见比它大的停止
>
> `pop(n)`：出队。队首出队，但是出队时要判断原队首是否在push时被删除了，如果被删除就不用出队了
>
> `max()`：获取单调队列最大值。队首即为最大值。
>
> 
