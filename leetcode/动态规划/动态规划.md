...
>     let res1 = fn(nums, 0, nums.length - 2);
>     // 情况3：有阔能包含尾元素，但必然不包含首元素
>     let res2 = fn(nums, 1, nums.length - 1);
...
>
>   * 如果在[0, i-1]的范围内，找到了j，使得`dp[j] + 1 > dp[i]`，说明找到了一个`更长`的递增子序列。
>
...
> * 在nums[i] > nums[j]前提下
>
>   * 如果在[0, i-1]的范围内，找到了j，使得`dp[j] + 1 == dp[i]`，说明找到了两个`相同`长度的递增子序列。
>
...
> 3. 初始化dp数组
>
> dp[i]：dp[i]记录了i之前（包括i）最长递增序列的长度。最小的长度也是1，所以dp[i]初始为1。
>
...
> 输入：[1,3,5,4,7]
>
> <img src="动态规划.assets/image-20220729103909828.png" alt="image-20220729103909828" style="zoom:67%;" />
>
> ```js
...
> 这题要求的是必须找到一个连续的，和第300题相比，前者子序列，后者连续递增序列
>
> dp五部曲
>
> > 1. 确定dp数组以及下标的含义
